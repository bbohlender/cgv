...
const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    {"name": "SequentialStep", "symbols": ["ws", (lexer.has("arrow") ? {type: "arrow"} : arrow), "ws", "PrimarySteps"], "postprocess": ([,,,primary]) => primary},
    ...
    {"name": "Switch", "symbols": [(lexer.has("switchSymbol") ? {type: "switchSymbol"} : switchSymbol), (lexer.has("ws") ? {type: "ws"} : ws), "Steps", "ws", (lexer.has("openCurlyBracket") ? {type: "openCurlyBracket"} : openCurlyBracket), "Switch$ebnf$1", "ws", (lexer.has("closedCurlyBracket") ? {type: "closedCurlyBracket"} : closedCurlyBracket)], "postprocess": ([,,value,,,cases]) => ({ type: "switch", cases: cases.map(({ caseValue }: any) => caseValue), children: [value, ...cases.map(({ steps }: any) => steps)] })},
    {"name": "SwitchCase", "symbols": ["ws", (lexer.has("caseSymbol") ? {type: "caseSymbol"} : caseSymbol), (lexer.has("ws") ? {type: "ws"} : ws), "Constant", (lexer.has("colon") ? {type: "colon"} : colon), "ws", "Steps"], "postprocess": ([,,,caseValue,,,steps]) => ({ caseValue, steps })}
  ],
