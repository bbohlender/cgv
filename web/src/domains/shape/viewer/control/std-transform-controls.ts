...
export class StdTransformControls<TCamera extends Camera = Camera> extends Object3D {
    ...
    public getMode = (): StdTransformControls["mode"] => this.mode

    public setMode = (mode: StdTransformControls["mode"]): void => {
        this.mode = mode
    }
...
class TransformControlsGizmo extends Object3D {
    ...
    constructor(length: number, depth: boolean) {
        ...
        const sphereGeometry = new SphereGeometry(0.035)

        const scaleHandleGeometry = new BoxGeometry(0.125, 0.125, 0.125)

        const lineGeometry = new BufferGeometry()
        lineGeometry.setAttribute("position", new Float32BufferAttribute([0, 0, 0, length, 0, 0], 3))
        ...
        const gizmoTranslate = {
            X: [
                [new Mesh(arrowGeometry, matRed), [length, 0, 0], [0, 0, -Math.PI / 2], null, "fwd"],
                [new Mesh(arrowGeometry, matRed), [length, 0, 0], [0, 0, Math.PI / 2], null, "bwd"],
                [new Line(lineGeometry, matLineRed)],
                [new Mesh(sphereGeometry, matRed)],
            ],
            Y: [
                [new Mesh(arrowGeometry, matGreen), [0, length, 0], null, null, "fwd"],
                [new Mesh(arrowGeometry, matGreen), [0, length, 0], [Math.PI, 0, 0], null, "bwd"],
                [new Line(lineGeometry, matLineGreen), null, [0, 0, Math.PI / 2]],
                [new Mesh(sphereGeometry, matGreen)],
            ],
            Z: [
                [new Mesh(arrowGeometry, matBlue), [0, 0, length], [Math.PI / 2, 0, 0], null, "fwd"],
                [new Mesh(arrowGeometry, matBlue), [0, 0, length], [-Math.PI / 2, 0, 0], null, "bwd"],
                [new Line(lineGeometry, matLineBlue), null, [0, -Math.PI / 2, 0]],
                [new Mesh(sphereGeometry, matBlue)],
            ],
        ...
        for (let i = 0; i < handles.length; i++) {
            ...
            if (handle.tag === "helper") {
                ...
                if (handle.name === "AXIS") {
                    ...
                    if (this.axis === "X") {
                        ...
                        if (
                            Math.abs(this.alignVector.copy(this.unitX).applyQuaternion(quaternion).dot(this.eye)) > 0.9
                        ) {
                            handle.visible = false
                        }
                    }
                    ...
                    if (this.axis === "Y") {
                        ...
                        if (
                            Math.abs(this.alignVector.copy(this.unitY).applyQuaternion(quaternion).dot(this.eye)) > 0.9
                        ) {
                            handle.visible = false
                        }
                    }
                    ...
                    if (this.axis === "Z") {
                        ...
                        if (
                            Math.abs(this.alignVector.copy(this.unitZ).applyQuaternion(quaternion).dot(this.eye)) > 0.9
                        ) {
                            handle.visible = false
                        }
                    }
            ...
            if (this.mode === "translate" || this.mode === "scale") {
                ...
                if (handle.name === "X" || handle.name === "XYZX") {
                    if (
                        Math.abs(this.alignVector.copy(this.unitX).applyQuaternion(quaternion).dot(this.eye)) >
                        AXIS_HIDE_TRESHOLD
                    ) {
                ...
                if (handle.name === "Y" || handle.name === "XYZY") {
                    if (
                        Math.abs(this.alignVector.copy(this.unitY).applyQuaternion(quaternion).dot(this.eye)) >
                        AXIS_HIDE_TRESHOLD
                    ) {
                        handle.scale.set(1e-10, 1e-10, 1e-10)
                        handle.visible = false
                ...
                if (handle.name === "Z" || handle.name === "XYZZ") {
                    if (
                        Math.abs(this.alignVector.copy(this.unitZ).applyQuaternion(quaternion).dot(this.eye)) >
                        AXIS_HIDE_TRESHOLD
                    ) {
                        handle.scale.set(1e-10, 1e-10, 1e-10)
                        handle.visible = false
                ...
                if (handle.name.search("X") !== -1) {
                    if (
                        this.alignVector.copy(this.unitX).applyQuaternion(quaternion).dot(this.eye) < AXIS_FLIP_TRESHOLD
                    ) {
                        if (handle.tag === "fwd") {
                            handle.visible = false
                        } else {
                ...
                if (handle.name.search("Z") !== -1) {
                    if (
                        this.alignVector.copy(this.unitZ).applyQuaternion(quaternion).dot(this.eye) < AXIS_FLIP_TRESHOLD
                    ) {
                        if (handle.tag === "fwd") {
                            handle.visible = false
                        } else {
